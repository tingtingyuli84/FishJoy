...
public class ArtilleryController implements GameConstants {
	...
	private TiledSprite createSingleButton(final Artillery_Operate operate, TiledTextureRegion pTiledTextureRegion, 
		...
		if(operate.equals(Artillery_Operate.WEAKEN))
			pX = CAMERA_WIDTH /2 - artilleryTextureRegion.getWidth()/10 - pTiledTextureRegion.getWidth()/2 +5;
		else
			pX = CAMERA_WIDTH /2 - artilleryTextureRegion.getWidth()/10 - pTiledTextureRegion.getWidth()/2 +121-30;
	...
	public void artilleryInitialization(TiledTextureRegion artilleryTextureRegion,
		...
		//about artillery
		int px = (CAMERA_WIDTH - artilleryTextureRegion.getWidth()/5)/2 + 1;
	...
	scene.getChild(LAYER_ARTILLERY).attachChild(leftButton);
	scene.getChild(LAYER_ARTILLERY).attachChild(rightButton);
